...
impl Default for ValidationConfig {
    fn default() -> Self {
        Self {
            max_payload_size_bytes: 1024, // 1KB - forces intentional sizing
            max_string_length: 1024,      // 1KB strings max
            max_object_depth: 5,          // Shallow nesting to prevent JSON bombs
...
impl PayloadValidator {
    ...
    pub fn validate(&self, payload: &serde_json::Value) -> Result<()> {
        // 1. Fast size check (serialize once)
        self.validate_size(payload)?;

        // 2. Rate limit check (very fast atomic operation)
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.max_payload_size_bytes, 1024);
