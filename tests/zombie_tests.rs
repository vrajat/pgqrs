use pgqrs::{
    config::Config,
    tables::{PgqrsWorkers, PgqrsMessages},
    types::WorkerStatus,
    admin::PgqrsAdmin,
    producer::Producer,
    consumer::Consumer,
...
async fn test_zombie_lifecycle_and_reclamation() -> anyhow::Result<()> {
    ...
    let zombie_count = workers_table.count_zombies_for_queue(queue.id, Duration::from_secs(60)).await?;
    assert_eq!(zombie_count, 1, "Should detect 1 zombie worker");

    let zombies = workers_table.list_zombies_for_queue(queue.id, Duration::from_secs(60)).await?;
    assert_eq!(zombies.len(), 1);
    assert_eq!(zombies[0].id, consumer_worker_id);

    // 7. Test admin.reclaim_messages
    let reclaimed = admin.reclaim_messages(queue.id, Some(Duration::from_secs(60))).await?;
    ...
    assert_eq!(stored_msg.consumer_worker_id, None, "Message should have no consumer");
    assert_eq!(stored_msg.read_ct, 1, "Read count should be preserved/incremented");

    // Verify worker is stopped
    let updated_worker = workers_table.get(consumer_worker_id).await?;
    assert!(matches!(updated_worker.status, WorkerStatus::Stopped), "Worker should be stopped");
    ...
    assert!(stdout.contains("Reclaimed 1 messages"), "CLI output mismatch: {}", stdout);

    let c2_worker = workers_table.get(c2_id).await?;
    assert!(matches!(c2_worker.status, WorkerStatus::Stopped), "Consumer 2 should be stopped by CLI");
