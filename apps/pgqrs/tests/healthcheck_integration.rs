...
async fn test_complete_health_workflow() {
    ...
    assert!(
        help_stdout.contains("liveness"),
        "Should list liveness command"
    );
    assert!(
        help_stdout.contains("readiness"),
        "Should list readiness command"
    );
    ...
    assert!(
        invalid_stderr.contains("Failed to create client") || invalid_stderr.contains("error"),
...
async fn test_cli_output_formatting() {
    ...
    let liveness_response = client
        .liveness()
        .await
        .expect("Should get liveness response");
    ...
    let readiness_response = client
        .readiness()
        .await
        .expect("Should get readiness response");
    ...
    assert!(
        readiness_json.contains("\"failing_services\""),
        "Should include failing services"
    );
...
async fn test_cli_argument_validation() {
    ...
    let quiet_test = Command::new("cargo")
        .args(["run", "-p", "pgqrs-cli", "--", "--quiet", "--help"])
        .output()
