...
async fn get_or_create_queue(store: &Store, queue_name: &str) -> pgqrs::types::QueueRecord {
    match store.queue(queue_name).await {
        Ok(q) => q,
        Err(pgqrs::error::Error::QueueAlreadyExists { .. }) => pgqrs::tables(store)
            .queues()
...
async fn get_or_create_workflow(store: &Store, wf_name: &str) -> pgqrs::types::WorkflowRecord {
    match store.workflow(wf_name).await {
        Ok(w) => w,
        Err(pgqrs::error::Error::WorkflowAlreadyExists { .. }) => pgqrs::tables(store)
            .workflows()
            .get_by_name(wf_name)
            .await
            .unwrap(),
...
async fn test_standalone_sql_job_success() {
    ...
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM test_job_table WHERE val = 'success_message'")
            .fetch_one(store.pool())
            .await
            .unwrap();
    assert_eq!(count, 1);
...
async fn test_sql_workflow_orchestration() {
    ...
    let run = pgqrs::tables(&store)
        .workflow_runs()
        .insert(pgqrs::types::NewRunRecord {
            workflow_id: wf.id,
            message_id: msg.id,
            input: Some(input),
        })
...
async fn test_sql_workflow_failure_rollback() {
    ...
    assert!(run.error.unwrap()["message"]
        .as_str()
        .unwrap()
        .contains("Fatal workflow error"));
}
