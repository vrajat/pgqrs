...
#[tokio::test]
async fn test_simple_message_lifecycle() {
    ...
    let producer = pgqrs::producer("test_simple_message_lifecycle-3001", TEST_QUEUE_ARCHIVE)
        .create(&store)
        .await
        .expect("Failed to create producer");

    let consumer = pgqrs::consumer("test_simple_message_lifecycle-3101", TEST_QUEUE_ARCHIVE)
        .create(&store)
        .await
        .expect("Failed to create consumer");

    // Dequeue before any messages exist
    let empty_messages = pgqrs::dequeue()
        .worker(&consumer)
        .batch(1)
        .fetch_all(&store)
        .await
    ...
    let empty_message = pgqrs::dequeue()
        .worker(&consumer)
        .fetch_one(&store)
        .await
        .expect("Failed to fetch_one from empty queue");
    assert!(empty_message.is_none());
    ...
    assert_eq!(dequeued_msgs[0].payload, payload, "Payload should match");
...
async fn test_archive_nonexistent_message() {
    ...
    let consumer = pgqrs::consumer(
        "test_archive_nonexistent_message-3103",
...
async fn test_validation_payload_size_limit() {
    let store = create_store_with_config(|config| {
        config.validation_config.max_payload_size_bytes = 50;
    })
    .await;
    let queue_info = pgqrs::admin(&store)
    ...
    let producer = pgqrs::producer(
        "test_validation_payload_size_limit-3007",
...
async fn test_validation_forbidden_keys() {
    let store = create_store_with_config(|config| {
        config.validation_config.forbidden_keys =
            vec!["secret".to_string(), "__proto__".to_string()];
    })
    .await;
    let queue_info = pgqrs::admin(&store)
        ...
        .unwrap();
    let producer = pgqrs::producer(
        "test_validation_forbidden_keys-3008",
...
async fn test_validation_required_keys() {
    ...
    let queue_info = pgqrs::admin(&store)
        ...
        .unwrap();
    let producer = pgqrs::producer(
        "test_validation_required_keys-3009",
...
async fn test_batch_validation_atomic_failure() {
    ...
    let queue_info = pgqrs::admin(&store)
        ...
        .unwrap();
    let producer = pgqrs::producer(
        "test_batch_validation_atomic_failure-3011",
...
async fn test_validation_string_length() {
    ...
    let queue_info = pgqrs::admin(&store)
        ...
        .unwrap();
    let producer = pgqrs::producer(
        "test_validation_string_length-3012",
...
async fn test_validation_accessor_methods() {
    ...
    let queue_info = pgqrs::admin(&store)
        ...
        .unwrap();
    let producer = pgqrs::producer(
        "test_validation_accessor_methods-3013",
...
async fn test_consumer_extend_visibility_behavior() {
    ...
    let msg = pgqrs::dequeue()
        .worker(&consumer)
        .with_vt(std::time::Duration::from_secs(1))
        .fetch_one(&store)
        .await
        .unwrap()
