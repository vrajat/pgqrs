use pgqrs::{Consumer, Producer, Config, Admin, Worker};
use serde_json::{json};
...
async fn create_admin() -> pgqrs::admin::Admin {
    ...
    if let Err(e) = sqlx::query("TRUNCATE TABLE pgqrs_workers, pgqrs_queues RESTART IDENTITY CASCADE")
        .execute(&admin.pool)
        .await
    {
...
async fn test_zombie_consumer_race_condition() {
    ...
    let queue_info = admin.create_queue(queue_name).await.expect("Failed to create queue");
    ...
    let msgs_a = consumer_a.dequeue_many_with_delay(1, 1).await.expect("Dequeue A failed");
    ...
    let released = admin.release_worker_messages(consumer_a.worker_id()).await.expect("Release failed");
    ...
    assert_eq!(msgs_b.len(), 1, "Consumer B should be able to pick up expired message");
    let msg_b = &msgs_b[0];
    ...
    assert!(!deleted_a, "Consumer A should NOT be able to delete message owned by B");
    println!("Consumer A delete correctly failed.");

    // 8. Consumer A tries to ARCHIVE -> Should FAIL (return None)
    let archived_a = consumer_a.archive(msg.id).await.expect("Archive A op failed");
    assert!(archived_a.is_none(), "Consumer A should NOT be able to archive message owned by B");
    ...
    assert!(deleted_b, "Consumer B should be able to delete its own message");
...
async fn test_zombie_consumer_batch_ops() {
    ...
    let producer = Producer::new(pool.clone(), &queue_info, "prod", 1, &config).await.unwrap();
    let consumer_a = Consumer::new(pool.clone(), &queue_info, "con_a", 2, &config).await.unwrap();
    let consumer_b = Consumer::new(pool.clone(), &queue_info, "con_b", 3, &config).await.unwrap();
    ...
    let released = admin.release_worker_messages(consumer_a.worker_id()).await.unwrap();
    ...
    let results_a = consumer_a.delete_many(vec![msg1.id, msg2.id]).await.unwrap();
    assert_eq!(results_a, vec![false, false], "Batch delete by A should fail for all");

    // A tries archive_many -> return [false, false]
    let arch_results_a = consumer_a.archive_many(vec![msg1.id, msg2.id]).await.unwrap();
    assert_eq!(arch_results_a, vec![false, false], "Batch archive by A should fail for all");

    // B deletes -> [true, true]
    let results_b = consumer_b.delete_many(vec![msg1.id, msg2.id]).await.unwrap();
    assert_eq!(results_b, vec![true, true], "Batch delete by B should succeed");
}
