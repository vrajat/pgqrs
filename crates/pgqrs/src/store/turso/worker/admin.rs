...
use crate::types::{QueueInfo, QueueMessage, QueueMetrics, SystemStats, WorkerHealthStats, WorkerInfo, WorkerStats, WorkerStatus};
...
impl Admin for TursoAdmin {
    async fn install(&self) -> Result<()> {
        ...
        for stmt in schema {
            conn.execute(stmt, ()).await.map_err(|e| crate::error::Error::TursoQueryFailed {
    ...
    async fn create_queue(&self, name: &str) -> Result<QueueInfo> {
         ...
         queue_table.insert(crate::types::NewQueue { queue_name: name.to_string() }).await
    }
    ...
    async fn purge_queue(&self, name: &str) -> Result<()> {
         ...
         conn.execute("DELETE FROM pgqrs_messages WHERE queue_id = ?", (queue.id,)).await.map_err(|e| crate::error::Error::TursoQueryFailed { query: "DELETE messages".into(), source: e, context: "Purge queue".into() })?;
         conn.execute("DELETE FROM pgqrs_workers WHERE queue_id = ?", (queue.id,)).await.map_err(|e| crate::error::Error::TursoQueryFailed { query: "DELETE workers".into(), source: e, context: "Purge queue".into() })?;
